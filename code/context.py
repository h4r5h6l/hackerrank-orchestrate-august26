...
def _extract_signals(message: dict[str, Any], content_text: str) -> dict[str, Any]:
	...
	return {
		...
		"deadline": bool(re.search(r"urgent|asap|before \d|by \d|before eod|eod|expires?|last[- ]minute|within \d+ minutes?|\d+ mins? max|in \d+ minutes?", text)),
		"payment": bool(re.search(r"\b(?:pay|payment|fee|transfer|refund|upi|invoice|charged|amount due)\b", text)),
		...
		"promotional": bool(re.search(r"sale|selling|for sale|offer|discount|deal|limited time|book now|rs\.?\s*\d|kurta set|\d+%\s*off|coupon", text)),
